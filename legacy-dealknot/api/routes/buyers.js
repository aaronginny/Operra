...
router.post("/", async (req, res) => {
  const { name, phone, dial, country, area, type, cur, min, max, intl, notes, division, period, label, radiusKm, referredBy } = req.body || {};
  ...
  await client.execute({
    sql: `INSERT INTO buyers (id, broker_id, name, phone, dial, country, area, type, cur, min_price, max_price, intl, notes, division, period, label, radius_km, referred_by)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [id, req.broker.id, name, phone, dial || "+1", country, area, type, cur,
           Number(min), Number(max), intl ? 1 : 0, notes || "", div, per, lab, rkm, referredBy || ""],
  });
...
router.put("/:id", async (req, res) => {
  ...
  const result = await client.execute({
    sql: `UPDATE buyers SET name=?, phone=?, dial=?, country=?, area=?, type=?, cur=?,
          min_price=?, max_price=?, intl=?, notes=?, division=?, period=?, label=?, radius_km=?, referred_by=?
...
router.post("/bulk", async (req, res) => {
  ...
  for (const rec of rows) {
    const { name, phone, dial, country, area, type, cur, min, max, intl, notes, division, period, label, referredBy } = rec;
    if (!name || !phone || !country || !area || !type || !cur || min == null || max == null) {
      failed++;
    ...
    try {
      await client.execute({
        sql: `INSERT INTO buyers (id, broker_id, name, phone, dial, country, area, type, cur, min_price, max_price, intl, notes, division, period, label, referred_by)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [id, req.broker.id, name, phone, dial || "+1", country, area, type, cur,
               Number(min), Number(max), intl ? 1 : 0, notes || "", div, per, lab, referredBy || ""],
      });
