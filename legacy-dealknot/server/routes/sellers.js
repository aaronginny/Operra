...
router.post("/", async (req, res) => {
  const { name, phone, dial, country, area, type, cur, price, intl, notes, division, period, label } = req.body || {};
  ...
  await client.execute({
    sql: `INSERT INTO sellers (id, broker_id, name, phone, dial, country, area, type, cur, price, intl, notes, division, period, label)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [id, req.broker.id, name, phone, dial || "+1", country, area, type, cur,
           Number(price), intl ? 1 : 0, notes || "", div, per, lab],
  });
...
router.put("/:id", async (req, res) => {
  ...
  const result = await client.execute({
    sql: `UPDATE sellers SET name=?, phone=?, dial=?, country=?, area=?, type=?, cur=?,
          price=?, intl=?, notes=?, division=?, period=?, label=?
...
router.post("/bulk", async (req, res) => {
  ...
  for (const rec of rows) {
    const { name, phone, dial, country, area, type, cur, price, intl, notes, division, period, label } = rec;
    if (!name || !phone || !country || !area || !type || !cur || price == null) {
      failed++;
